...
def inference(args, model, test_save_path=None):
    ...
    writer = SummaryWriter(f'/scratch_net/biwidl217_second/arismu/Tensorboard/2022/FETS/{model_type}/TTA/TEST/{tta_type}/' + f'SD_FETS_{model_type}_log_seed{seed}_da{data_aug}_REPORT')
    ...
    gtts[np.where(gtts == 4)] = 3 #turn labels [0 1 2 4] into [0 1 2 3]
    ...
    for sub_num in range(num_test_subjects):
        ...
        if measure_calibration:
            if sub_num < 10: #limit to 10 subjects for memory reasons
    ...
    if measure_calibration:
        ...
        fig.savefig(f'/scratch_net/biwidl217_second/arismu/Data_MT/plots/FETS_SD_calibration_{model_type}_da{data_aug}_seed_{seed}.png')
...
if __name__ == "__main__":
    ...
    test_save_dir = f'../predictions_2022/FETS/{model_type}/TTA/{tta_type}/'
    test_save_path = os.path.join(test_save_dir, f'SD_FETS_{model_type}_test_seed{seed}_da{data_aug}')
